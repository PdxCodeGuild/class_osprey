...
What is the new entry\'s... ''')
        ...
        for entry in new_list:
            ...
            for key in headers:
                ...
                i += 1
        all_contacts.append(contact_info)
        print(f'Success! {c_name} is now in your contacts.')
        break
...
#-----------------------UPDATE
    while choice == 'U':
        ...
        u_name = input('Whose information are you updating? ').title()
...
#-----------------------DELETE
    while choice == 'D':
        ...
        d_name = input('Whose record would you like to remove? ').title()
        for match in range(len(all_contacts)):
